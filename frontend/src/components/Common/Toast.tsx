...
export function Toast({ message, onDismiss, tone = 'gold' }: ToastProps) {
  const toneClass =
    tone === 'alert'
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 z-50 max-w-sm animate-fade-up rounded-lg border px-4 py-3 text-sm shadow-panel backdrop-blur ${toneClass}`}
      role="status"
    >
